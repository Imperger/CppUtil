...
void test_standard_deviation() { test(apply<double>(util::standard_deviation, {10, 20}) == 5); }
...
void test_max_subarray_sum()
{
    ...
    }
    {
        ...
        util::max_subarray_result<std::vector<int>::iterator> r{std::next(a.begin(), 5), std::next(a.begin(), 7), 21};
...
void test_longest_common_subsequence()
{
    ...
    t3.expect = {t3.a.begin() + 1, t3.a.begin() + 2, t3.a.begin() + 5, t3.a.begin() + 6};
...
void test_threadsafe_queue()
{
    ...
    {
        threadlist.push_back(std::thread([&, n] { q.push(n); }));
    }
...
{
    {
        ...
        auto result = util::levenshtein_distance(a.begin(), a.end(), b.begin(), b.end());
...

    {
        ...
        auto result = util::levenshtein_distance(a.begin(), a.end(), b.begin(), b.end(), {2, 2, 1});
...
int main()
{
    try
    {
        ...
        random_string_test(util::random_string<std::string>::hex(),
                           [](char x) { return (x >= '0' && x <= '9') || (x >= 'a' && x <= 'f'); });
        random_string_test(util::random_string<std::wstring>::hex(),
                           [](char x) { return x >= ('0' && x <= '9') || (x >= 'a' && x <= 'f'); });
        test_utf8_iterator();
        test_timing();
        test_print_memory();
